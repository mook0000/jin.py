...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        ##### 종목분석용 데이타
        self.calcul_data = []
        self.wb_test = None         #엑셀 파일명 변수
        self.ws1 = None             #엑셀 시트명 변수
        #############################
    ...
    def get_interest_stock(self):
        #self.in_file = open("C:/MYPrograms/pythonfiles/lims/files/interest.txt", "r")
        #wb = load_workbook('./files/Test.xlsm', keep_vba=True)
        #count = 0
        #self.inter
        self.iw = pd.read_excel("C:/MYPrograms/pythonfiles/lims/files/interest.xlsx",index_col=None,dtype={'종목코드':str,'종목명':str})
        #  키움증권 관심종목 1
        for row in self.iw.index:
            code = self.iw.iloc[row,8]
            self.run_codenm = self.iw.iloc[row,2]
        ...
        self.wb_test = xw.Book("C:/MYPrograms/pythonfiles/lims/files/stockdata.xlsm")
        self.ws1 = self.wb_test.sheets['data01']
        df.set_index('종목명', inplace=True)
        self.ws1.range('2:1000').api.Delete()
        self.ws1.range("A1").value = df
        # QTest.qWait(5000)
    ...
    def ref_kiwoom_db(self):
        print("투자자현황")
        for row in self.iw.index:
            self.run_code = code = self.iw.iloc[row,8]
            self.run_codenm = self.iw.iloc[row, 2]
            print(code)
        ...
        column = ["종목명","code","일자","현재가","대비기호","전일대비","등락율","누적거래량","누적거래대금","개인투자자","외국인투자자","기관계",
                  "금융투자","보험","투신","기타금융","은행","연기금등","사모펀드","국가","기타법인","내외국인"]     #  'codenm",
        df =  pd.DataFrame(self.calcul_data,columns=column)
        #wb_test = xw.Book("C:/MYPrograms/pythonfiles/lims/files/report.xlsm")
        self.ws1 = self.wb_test.sheets['data02']
        df.set_index('종목명', inplace=True)
        self.ws1.range('2:10000').api.Delete()
        self.ws1.range("A1:v10000").value = df
    ...
    def trdata_slot(self,sScrNo,sRQName,sTrCode,sRecordName,sPrevNext):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            temp_dict.update({'종목명': self.run_codenm})
            temp_dict.update({'code': code.strip()})
            temp_dict.update({'기준일': datetime.today()})
            ...
            for i in range(cnt):
                ...
                #data.append("")
                data.append(self.run_codenm)
                data.append(self.run_code )                    # 코드명 삭제 --> 위 컬럼명 조정
